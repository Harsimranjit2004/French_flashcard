...
type Card = { id:string; noteId:string; kind:Kind; memory:FSRSCard; lapses:number; successes:number; suspended:boolean };
...
function Review({store,dispatch,done}:{store:Store;dispatch:React.Dispatch<Action>;done:()=>void}){
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.key===" "){e.preventDefault();setShown(true)}if(e.key.toLowerCase()==="r")play();if(e.key.toLowerCase()==="m")mic();if(shown&&["1","2","3","4"].includes(e.key))rate([Rating.Again,Rating.Hard,Rating.Good,Rating.Easy][+e.key-1] as Grade)};addEventListener("keydown",key);return()=>removeEventListener("keydown",key)});
